...
def makePlayer():
    ...
    print("\n你的游戏主角名字叫 {0}. 你的生命值是满的 ({1} 点), 你有 {2} 个金币. 你身上没有任何东西.".format(player["name"], player["hp"],
                                                                             player["gold"]))
    print(
        "\t%s 是一个一级角色, 需要获得 %s 点经验才能升级. 这样做我们会获得更多最大生命值. 你可以在探索和战斗中获得经验." % (player["name"], player["xpForNextLevel"]))
...
def description():
    ...
    print("(X: " + str(player["locationX"]) + ", Y: " + str(player["locationY"]) + ") 该区域状态： " + isExplored + "." + str(
...
def getAction():
    ...
    # 探索地区
        ...
        print(player['name'] + ' /等级: ' + str(player['level']) + "级  经验值: " + str(player["xp"]) + "点 (还需要" + str(
            player["xpForNextLevel"]) + "点升级)")
